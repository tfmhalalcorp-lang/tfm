...
const resource = createTxnResource({
  ...
  toPayload: (form) => ({
    ...
    rm_price: form.rm_price === '' ? null : Number(form.rm_price),
  }),
  loadExtra: async () => {
...
registerView('prod-rm', async (container) => {
  container.innerHTML = txnListTemplate({
    ...
    theadHtml: `<th class="px-3 py-2">วันที่</th><th class="px-3 py-2">Batch</th><th class="px-3 py-2">Supplier</th><th class="px-3 py-2">เลขที่บิลซื้อ</th><th class="px-3 py-2">ปลา</th><th class="px-3 py-2 text-right">น.น.(kg)</th><th class="px-3 py-2 text-right">คงค้าง (kg.)</th><th class="px-3 py-2 text-right">ปลาคงค้าง (KG)</th><th class="px-3 py-2 text-right" x-show="$store.auth.role === 'admin'">ราคา RM</th>`,
    rowHtml: `
      ...
      <td class="px-3 py-2 text-right" x-show="$store.auth.role === 'admin'" x-text="item.rm_price != null ? Number(item.rm_price).toLocaleString(undefined,{minimumFractionDigits:2}) : '-'"></td>
    `,
    colCount: 9,
    ...
    modalFieldsHtml: `
      ...
      <template x-if="$store.auth.role === 'admin'">
        <div><label class="form-label">ราคา RM (บาท/กก.)</label><input type="number" step="0.01" min="0" x-model="form.rm_price" class="form-control"></div>
      </template>
    `,
