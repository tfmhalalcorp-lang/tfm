...
create table public.profiles (
  ...
  role       text not null check (role in ('admin','pd','wh','qc','ma','sale')),
...
-- 'sale' role) when no user_metadata is supplied — some Supabase Dashboard
...
-- Empty (unfilled) can usage — tracks cans received from a supplier, by
-- can size, independent of any production batch (no batch_id, unlike the
-- other prod_* tables above).
...
create table public.qc_waste (
  ...
  -- batch_id/cansize_id are nullable because the 'MA' (Maintenance) waste
  -- log below doesn't use them — it only records a machine + a plain
  -- quantity, via machine_id/waste_qty.
  batch_id        text references public.prod_batches(batch_id) on delete restrict,
  department      text not null check (department in ('PD','WH','QC','MA')),
...
-- --- prod_can: admin, pd, qc (shared form used by both PRODUCTION and QC menus) ---
...
-- --- qc_waste: admin, or pd/wh/qc/ma but ONLY for rows whose department
--     matches their own department (PD/WH/QC/MA) — coupled on insert AND
...
-- --- issue_log: any of admin/pd/wh/qc/ma may INSERT for any department
...
revoke all on public.profiles, public.customers, public.brands, public.suppliers,
  public.can_sizes, public.machines, public.sauces, public.prod_batches, public.prod_rm,
  public.prod_fillw, public.prod_can, public.prod_fillq, public.prod_emptycan, public.qc_sauce,
  public.wh_in, public.qc_waste, public.machine_pm, public.issue_log
  from anon;

grant select, insert, update, delete on
  public.customers, public.brands, public.suppliers, public.can_sizes, public.machines, public.sauces,
  public.prod_batches, public.prod_rm, public.prod_fillw, public.prod_can, public.prod_fillq,
  public.prod_emptycan, public.qc_sauce, public.wh_in, public.qc_waste, public.machine_pm, public.issue_log
  to authenticated;
...
--    matching `profiles` row automatically. Every other account (pd,
--    wh, qc, ma, sale users) should then be created from inside the app's
